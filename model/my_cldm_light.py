...
class ControlLDM(LatentDiffusion):
    ...
    def __init__(self, control_stage_config, hint_key, only_mid_control,
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        # --- Get the hint input and fix its dimension order ---
        hint_input = batch[self.hint_key]
        if bs is not None:
            hint_input = hint_input[:bs]
        hint_input = hint_input.to(self.device)

        # Permute tensor from (B, H, W, C) to (B, C, H, W) for PyTorch's convolution layers.
        if hint_input.ndim == 4 and hint_input.shape[-1] in [1, 3]:
            hint_input = hint_input.permute(0, 3, 1, 2)
            
        control = hint_input.to(memory_format=torch.contiguous_format).float()
        
        cond = {
            "c_concat": [control],
            "c_crossattn": [c], # Use pure text conditioning
        }
        return x, cond
